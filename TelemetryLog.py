...
def get_log():
    ...
    while(myCar.loop):
        while(indice < len(myCar.log)):
            mnsSend = "{}".format(myCar.log[indice][0])

            mnsRecv = "{}".format(myCar.log[indice][1])
            print("Este es enviado",mnsSend)
            print("Este es recibido",mnsRecv)
...
def send (event):
    ...
    """
    mns = str(E_Command.get())###LE VAMOS A MANDAR ESTO AL CARRO
...
def sendShowID():
    ...
    """
    mns = str(E_Command.get())## ESTO ES LO QUE ESCRIBO
    if(len(mns)>0 and mns[-1] == ";"):
        E_Command.delete(0, 'end') #LO BORRO
        mnsID = myCar.send(mns) #LO MANDO
