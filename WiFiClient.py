...
class NodeMCU(Thread):
    ...
    def __init__(self, ip = '192.168.43.200', port = 7070):
        ...
        self.node_address = (ip,port) #ASIGNA LA IP AL CARRO 
    ...
    def run(self):
        ...
        Anotación: el socket es un concepto que permite la comunicación entre dos programas 
        """
        self.loop = True
        while(self.loop):
            if(len(self.pending_mns)>0):
                ...
                for i in self.pending_mns:
                    message+=i
                    self.pending_mns.remove(i)#QUITA EL MENSAJE DE LA LSITA DE 
                message+="\r"
